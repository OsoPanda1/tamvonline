// ═══════════════════════════════════════════════════════════════
// TAMV MD-X4™ — PROTOCOL XR VISUAL TRANSLATION LAYER
// ═══════════════════════════════════════════════════════════════
//
// Este módulo define CÓMO SE VE EL PODER cuando TAMV actúa.
//
// No renderiza. No dibuja. No inventa formas.
//
...
// ═══════════════════════════════════════════════════════════════
// CAPA 5 — PRESENCIA DE PROTOCOLO EN ESCENA
// ═══════════════════════════════════════════════════════════════

export interface XRProtocolPresence {
  beaconDensity: "none" | "sparse" | "clustered" | "saturated";
  beaconColor: "blue" | "amber" | "red" | "emerald";
  institutionalActivity: "dormant" | "active" | "heightened";
  guardianPresence: "invisible" | "subtle" | "visible" | "prominent";
}

// ═══════════════════════════════════════════════════════════════
// CAPA 6 — TONO EMOCIONAL GLOBAL
// ═══════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════
// CAPA 7 — HYPER REAL SCENE FEDERADA
// ═══════════════════════════════════════════════════════════════
...
export interface XRHyperRealScene {
  ...
  scene: {
    protocolPresence: XRProtocolPresence;
  };
  affect: XRAffect;
...
// ═══════════════════════════════════════════════════════════════
// CAPA 8 — MOTOR DE TRADUCCIÓN (NÚCLEO FEDERADO)
// ═══════════════════════════════════════════════════════════════
...
export class ProtocolXRVisualEngine {
  ...
  public manifest(event: GuardianObservableEvent): XRHyperRealScene {
    ...
    return {
      ...
      scene: {
        protocolPresence: this.protocolPresenceFor(event),
      },
  ...
  private protocolPresenceFor(event: GuardianObservableEvent): XRProtocolPresence {
    const level = event.visual.level;
    
    return {
      beaconDensity: 
        level === "critical" ? "saturated" :
        level === "alert" ? "clustered" :
        level === "attention" ? "sparse" :
        "none",
      beaconColor:
        level === "critical" ? "red" :
        level === "alert" ? "amber" :
        level === "attention" ? "blue" :
        "emerald",
      institutionalActivity:
        event.state === "active" ? "heightened" :
        event.state === "arming" ? "active" :
        "dormant",
      guardianPresence:
        event.authority === "eoct" ? "prominent" :
        level === "critical" ? "visible" :
        level === "alert" ? "subtle" :
        "invisible",
    };
  }
...
// ═══════════════════════════════════════════════════════════════
// PRESETS DE MUNDO PARA REACT THREE FIBER
// ═══════════════════════════════════════════════════════════════
...
export function worldPresetFromScene(scene: XRHyperRealScene): WorldPresetId {
  switch (scene.macroScene) {
    case "exceptional_state":
      return "city_night_storm";
    case "threat_contained":
      return "city_night_rain";
    case "active_repair":
      return "city_dawn_cloudy";
    case "historical_record":
      return "city_dusk_clear";
    case "normal_civil_life":
    case "silent_alignment":
    default:
      return "city_day_clear";
  }
}

export const WORLD_PRESETS: Record<WorldPresetId, WorldPreset> = {
  city_day_clear: {
    hdriPath: "/hdris/city_day.hdr",
    exposure: 1.1,
    fog: { enabled: true, color: "#b0c4de", near: 50, far: 600 },
    ambientIntensity: 0.4,
    directionalIntensity: 1.0,
  },
  city_night_storm: {
    hdriPath: "/hdris/city_night_storm.hdr",
    exposure: 0.6,
    fog: { enabled: true, color: "#050711", near: 10, far: 400 },
    ambientIntensity: 0.1,
    directionalIntensity: 0.2,
  },
  city_dawn_cloudy: {
    hdriPath: "/hdris/city_dawn_cloudy.hdr",
    exposure: 0.9,
    fog: { enabled: true, color: "#8899aa", near: 30, far: 500 },
    ambientIntensity: 0.3,
    directionalIntensity: 0.7,
  },
  city_night_rain: {
    hdriPath: "/hdris/city_night_rain.hdr",
    exposure: 0.7,
    fog: { enabled: true, color: "#11151b", near: 20, far: 450 },
    ambientIntensity: 0.15,
    directionalIntensity: 0.3,
  },
  city_dusk_clear: {
    hdriPath: "/hdris/city_dusk.hdr",
    exposure: 0.85,
    fog: { enabled: true, color: "#2a1f3d", near: 40, far: 550 },
    ambientIntensity: 0.25,
    directionalIntensity: 0.5,
  },
};

// Singleton del motor de traducción
export const xrVisualEngine = new ProtocolXRVisualEngine();
